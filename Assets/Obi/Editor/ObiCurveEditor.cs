		...
		protected void DrawControlPoint(int i, bool selected){
			...
			if (selected && !orientControlPointsMode){
				...
				if (!(i == 0 && !spline.closed)){
					...
					EditorGUI.BeginChangeCheck();
					var fmh_332_68_639126212089860445 = Quaternion.identity; Vector3 newTangent = Handles.FreeMoveHandle(cp.GetInTangent(),size,Vector3.zero,Handles.DotHandleCap);
					if (EditorGUI.EndChangeCheck()){
				...
				if (!(i == spline.controlPoints.Count-1 && !spline.closed)){
					...
					EditorGUI.BeginChangeCheck();
					var fmh_345_69_639126212089864766 = Quaternion.identity; Vector3 newTangent = Handles.FreeMoveHandle(cp.GetOutTangent(),size,Vector3.zero,Handles.DotHandleCap);
					if (EditorGUI.EndChangeCheck()){
						Undo.RecordObject(spline, "Modify tangent");
